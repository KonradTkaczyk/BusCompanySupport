...
def make_tickets (admin)
  ...
  10.times do |m|
    ...
    10.times do |n|
      admin.tickets.create!(:dateOfTrip => time + 2.days + m.days + 18.hours, :endOfTrip => time + 2.days + m.days + 20.hours, :bus_id => 8, :cityFrom => "Cracow", :cityTo => "Wroclaw", :nameOfSeat => "#{n+1}", :trip => "#{m*23+18}")
    end
    10.times do |n|
